...
class MAMLPatch(torch.nn.Module):
    ...
    """
    def __init__(self, inner_lr: float = 0.5, adapt_steps: int = 10):
        super().__init__()
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        with higher.innerloop_ctx(self.net, inner_opt) as (fnet, diffopt):
            for _ in range(self.adapt_steps):
                ...
                loss_sup = F.binary_cross_entropy(preds_sup.view(-1), support_y.float())
                diffopt.step(loss_sup)
            # Query loss after adaptation
            preds_q = fnet(query_x)
            loss_q = F.binary_cross_entropy(preds_q.view(-1), query_y.float())
        return loss_q
